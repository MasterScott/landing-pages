...
imoney.browser.technologies = (function(){
  var hasAdblock,
  ...
  function readValues()
  {
    var $elements = $('.advblock, .AdSense');
    hasAdblock = $elements.length && $elements.not(':visible').length > 0;
  }
  ...
  var public = {
    getAsString: function()
    {
      ...
      var techs = {};
      techs.a = hasAdblock ? 1 : 0;
      techs.sh = screenHeight || 0;
      techs.sw = screenWidth || 0;

      var values = [];
      for (var p in techs) {
        values.push(p + ':' + techs[p]);
      }
      return values.join('|');
    }
  };
...
var LocationHash = (function() {
  ...
  function listen()
  {
    ...
    if (newHash !== oldHash) {
      oldHash = newHash;
      EE.emitEvent('hash.change', [ newHash ]);
    }
  }
  ...
  var methods = {
    'init': function(options) {
      return this.each(function(){
        ...
        if (!data){
          $('.submit', $this).click(function(event){
          ...
          $('input[name="technologies"]', $this).val(imoney.browser.technologies.getAsString());
  ...
  function formSubmitHandler($form, callback)
  {
    ...
    $.post($form.attr('action'), $form.serialize(),
      function(data){
        ...
        $message.show();
        $('input, textarea, select, .btn', $form).removeAttr('disabled');
        $('.ico-loading', $form).css({visibility: 'hidden'});
      },
      'json');
    $('input, textarea, select, .btn', $form).attr('disabled', 'disabled');
    ...
    'init': function(options) {
      return this.each(function(){
        ...
        if (!data){
          ...
          $('input[name="technologies"]', $form).val(imoney.browser.technologies.getAsString());
          /* /Plugin body */
...
(function($){
  "use strict";
  $.fn.firstTimeGreeting = function(options)
  {
    var settings = {
      url: '',
    };
    ...
    return this.each(function(){
      ...
      var $alert = $(this);
      $('[data-dismiss="' + $alert.attr('id') + '"]', $alert).click(function(event){
        event.preventDefault();
        $alert.hide();
        $.get(settings.url);
      });
...
(function($){
  $.fn.simpleCountdown = function(options)
  {
    var settings = {
      ...
      // Минимальное количество секунд для запоминающего таймера
      minimumLeft: 5 * 60,

      // Название таймера, используется для запоминающего таймера
      name: 'countdown',

      /*
       * Запоминать время первого запуска на это количество дней
       * и отсчитывать таймер с первого запуска.
       */
      remember: 30,
    ...
    return this.each(function(){
      ...
      function init()
      {
        if (settings.remember) {
          var timeoutStart = imoney.cookie.read(cookieName);
          if (!timeoutStart) {
            timeoutStart = (new Date()).toString();
            imoney.cookie.create(cookieName, timeoutStart, settings.remember);
          }
          var startDate = new Date(timeoutStart);
          var finishDate = new Date();
          finishDate.setTime(startDate.getTime() + settings.timeLeft * 1000);

          var nowDate = new Date();
          var timeLeft = Math.round((finishDate - nowDate) / 1000);

          if (timeLeft < settings.minimumLeft) {
            timeLeft = settings.minimumLeft;
          }
 ...
 *
 *  2.0.2 Убирать iframe при показе попапа
 *
 *  @version 2.0.2
 */
var exitPopup = (function()
{
  ...
  function _beforeUnloadHandler()
  {
    if(!_skip){
      ...
      $('iframe[name!=' + _settings.frameName + ']').remove();
...
$(function(){
  ...
  $('a.btn', $actionsBox).click(function(event){
    event.preventDefault();
