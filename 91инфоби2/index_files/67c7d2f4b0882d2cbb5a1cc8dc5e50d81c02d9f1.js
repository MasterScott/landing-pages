/*ROOT/web/js/jquery-1.9.1.min.js*/
...
/*ROOT/web/js/jquery-migrate-1.1.1.min.js*/
...
/*ROOT/libs/bootstrap/js/bootstrap.min.js*/
/**
...
function getUrlVars()
{
  var v = window.location.href.slice(window.location.href.indexOf('?') + 1);
  if (''==v || window.baseUrl==v){
    v=null;
    return {};
  }

  var res = {}, e, hashes = v.split('&');
...
$(document).on('submit','form.frm_lead',function(){
  var id = $(this).attr('id');
  if (!id) id = $(this).attr('frm-id');

  var frm=$(this), inp_track=frm.find('input.inp_track');
  if (!inp_track.length) {inp_track=$('<input type="hidden" class="inp_track" name="inp_track" value=""/>'); inp_track.appendTo(frm);}
  inp_track.val(escapeRus(getCookie('bm360track')));
//  if (!inp_utm.length) {inp_utm=$('<input type="hidden" class="inp_utm" name="inp_utm" value=""/>'); inp_utm.appendTo(frm);}
//  inp_utm.val(escapeRus(getCookie('bm360utm')));

  if (!frm.find('input.inp_utm').length){
    $('<input type="hidden" class="inp_utm" name="has_utm" value="1"/>').appendTo(frm);
    var params=getUrlVars();
    for (var k in params){
      $('<input type="hidden" class="inp_utm" name="'+k+'" value="'+params[k]+'"/>').appendTo(frm);
    }
    params=k=null;
  }
  ...
  $.ajax({
    ...
    beforeSend: function(jqXHR, settings){
      var btn=settings.frm.find('input.ui_btn, input.btn-submit');//.attr('disabled','disabled').addClass('disabled').val('Сохраняем...');
      if (!btn.data('button'))
      {
        var lang = btn.attr('lang');
        btn.button({'loadingText': lang && 'en'==lang ? 'Loading...' : 'Загрузка...', 'defText': btn.val()});
        lang=null;
      }
      btn.button('loading');
    },
    success: function(data){
      this.frm.find('.ui_has_error').removeClass('ui_has_error');
      this.frm.find('.ui_error').hide().html('');
      this.frm.find('input.ui_btn, input.btn-submit').button('def');

      if (isset(data.error))
      {
        var attr=null,attrErr=null,firstErr=false,attrId=null;
        for (attrId in data.error)
        {
          attr=this.frm.find('#'+this.frm.attr('id')+'__'+attrId);
          attrErr=this.frm.find('.ui_error[for='+this.frm.attr('id')+'__'+attrId+']')
          if (attr.length)
          {
            attr.addClass('ui_has_error');
            if (false===firstErr){attr.focus(); firstErr=true;}
          }
          if (attrErr.length){attrErr.show(); attrErr.html('<div>'+data.error[attrId].join('</div><div>')+'</div>');}
        }
        attr=attrId=attrErr=firstErr=null;
      }

      if (isset(data.exception) && window.dbgMode)
      {
      ...
      if (isset(data.script))
      {
        ...
        catch (ex) { if (window.console && window.dbgMode){console.log(ex); console.log('[_Ошибка JS_] '+ex.stack);} }
      }
      ...
      if (isset(data.res))
      {
        if (1==data.res)
        {
          reachGoalByAttr(this.frm, 'goal-submitted');

          switch (parseInt(this.frm.attr('data-mes-url'))){
            case 1:
              var mes = this.frm.attr('suссess-message');
              if (isset(data.lead_preview)) {mes+=data.lead_preview;}
              if (mes.length) alert(mes);
              mes=null;
              break;

            case 2:
              var loc=this.frm.attr('data-goto-url');
              if (loc.length) {
                if (/^http(|s):\/\//.test(loc) !== true) {loc = 'http://'+loc;}
                  if (-1!==loc.indexOf('{name}')){
                      //console.log(this.frm.find('.field.fname:first').val());
                      loc = loc.replace('{name}',encodeURIComponent(this.frm.find('.field.fname:first input.inp').val()));
                  }
                  if (-1!==loc.indexOf('{order_id}') && data.id_lead){
                      //console.log(this.frm.find('.field.fname:first').val());
                      loc = loc.replace('{order_id}',data.id_lead);
                  }
            ...
            this.frm.find('input.inp,textarea.inp').val('');

            var w=this.frm.closest('.wind');
          if (w.length)
          {
            w.wind('hide');
          }
      ...
      //this.frm.find('input.ui_btn, input.btn-submit').removeAttr('disabled').removeClass('disabled');
    ...
    error: function(jqXHR, textStatus, errorThrown){
      ...
      //this.frm.find('input.ui_btn, input.btn-submit').val('Сохранить снова').removeAttr('disabled').removeClass('disabled');
      this.frm.find('input.ui_btn, input.btn-submit').button('def');
    }
  });
  id=null;
...
{
  ops= $.extend({html:true, showCloseBtn:false, width:false, okText:false, btnCls:'ibtn-blue', btnWidth:100, onOk:function(){}, headerText:false }, ops);
  var w=$('#jAlert');
  if (w.length)
  {
    if (false!==ops.headerText){
      w.find('.iblk_header').show().find('h5').text(ops.headerText);
    } else {
      w.find('.iblk_header').hide();
    }

    w.find('.wind-btn-apply')
      .text(ops.okText||'OK')
      .attr('class','ibtn wind-btn-apply '+ops.btnCls)
      .css('width',ops.btnWidth);

    if (true===ops.html) w.find('#jAlertBody').html(mes);
    else w.find('#jAlertBody').text(mes);

    w.find('.wind-btn-apply').unbind('click').click(function(){
      ops.onOk();
      $('#jAlert').wind('hide');
    });

    if (false!==ops.width) {w.css({width:ops.width});}
//    w.find('.wind-close')[true===ops.showCloseBtn ? 'show' : 'hide']();
    w.wind('show');
  }
  w=null;
}
...
function jConfirm(mes, ops)
{
  ...
  if (w.length)
  {
    ...
    w.find('.wind-close')[ops.showClose ? 'show' : 'hide']();
...
function showDoneBox(msg, opts)
{
  ...
  opts = $.extend({width:380,top:'40%',fadeIn:100, showTime:2000, fadeOut:500, cls:'', el:el}, opts);
  el.css({width: opts.width, 'margin-left': -1*Math.round(opts.width/2), top: opts.top, opacity:0});
  if (opts.cls.length){el.addClass(opts.cls);}
  el.appendTo($('body'));
...
function if_var_val(v,arr,def){
  for (var k in arr){
    if (v==k){k=null; return arr[k];}
  }
  k=null;
  return def||false;
}
...
function showExpiredWind(token)
{
  var frm=$('#sessionExpired form#frmlogin');
//  if ('undefined'===typeof frm.attr('inited')){
//    frm.submit(function(){
//      var inp=$('#sessionExpired #frmlogin__password'), token=$('#sessionExpired #frmlogin__magic'), v=inp.val();
//      console.log(inp);
//      console.log('pas',v);
//      console.log('sha1(pass)',$.sha1(v));
//      console.log('token',token.val());
//      inp.val($.sha1($.sha1(v)+token.val()));
//      console.log('res',inp.val());
//      inp=v=token=null;
//      return true;
//    });
//    frm.attr('inited',1);
//  }
  frm.find('#frmlogin__magic').val(token);
  $('#sessionExpired').wind('show').find('#frmlogin__username').focus();
//  if ('function'===typeof callback) {callback();}
}
function ExpiredWindbeforeSend()
{
  var inp=$('#sessionExpired #frmlogin__password'), token=$('#sessionExpired #frmlogin__magic');
//  console.log(inp);
//  console.log('pas',v);
//  console.log('sha1(pass)',$.sha1(v));
//  console.log('token',token.val());
  inp.val($.sha1($.sha1(inp.val())+token.val()));
//  console.log('res',inp.val());
  inp=token=null;
}
...
function ajaxLoad(ops)
{
  return $.ajax({
    ...
    beforeSend: function(jqXHR, settings){
      if (this.ops.onBeforeSend && 'function'==typeof this.ops.onBeforeSend){this.ops.onBeforeSend(this.ops);}
    },
    success: function(data){
      if (this.ops.onBeforeSuccess && 'function'==typeof this.ops.onBeforeSuccess){if (!this.ops.onBeforeSuccess(data, this.ops)){return};}
...
window._eh={step:20,speed:300,$container:false,$body:false,$wnd:false,bodyWidth:0,$img:null,$newImg:null,curSize:null,origSize:null,wndSize:null,newSize:null};//enlarge helper
function enlargeImage(el){
  //enlargeImage('1ffdc649797a46bdb1f9b2c091f54993') portait min
  //enlargeImage('11a33ab6b7834e9ab4d1916e370764c7') portait big

  //enlargeImage('13fc6e0204f2452aa8027acc842148fe') album min
  //enlargeImage('dd07aba1b8d24a7a98e615ce1957b99f') album big

  if (!_eh.$body){ _eh.$body=$('body'); }
  if (!_eh.$wnd){ _eh.$wnd=$(window); }

  if (!_eh.$container){
    _eh.$container = $('<div id="image-enlarge-cont"></div>');
    _eh.$container.appendTo(_eh.$body);
    _eh.$container.click(function(){
      if (!_eh.$newImg) return;
      _eh.$newImg.animate({top:_eh.curSize.y, left:_eh.curSize.x, width: _eh.curSize.w, height: _eh.curSize.h}, _eh.speed, function(){
        _eh.$container.hide();
        $('html').removeClass('wind_opened');
        _eh.$body.css('margin-right',0);
        _eh.$newImg.remove();
        _eh.$newImg=false;
      });
    })
  ...
  _eh.$img = $(el);
  if (!_eh.$img.length){return;}

  var o=_eh.$img.offset();
  _eh.curSize={w:_eh.$img.width(), h:_eh.$img.height(), x: o.left, y: o.top-scrollTop()};
  o=null;

  var i=new Image();
  i.src = _eh.$img.attr('src');
  _eh.origSize={w:i.width, h:i.height};
  _eh.$newImg = $(i);
  i=null;

  _eh.wndSize={w:_eh.$wnd.width() - 2 * _eh.step, h:_eh.$wnd.height() - 2 * _eh.step};
  _eh.newSize={w:0,h:0,x:0,y:0};

  console.log(_eh.origSize);
  if (_eh.origSize.w > _eh.wndSize.w && _eh.origSize.h > _eh.wndSize.h)
  {//если картинка больше экрана в оба направления, то делаем ее ширину = экрану
    _eh.origSize.h = _eh.wndSize.w*_eh.origSize.h/_eh.origSize.w;
    _eh.origSize.w = _eh.wndSize.w;
  }
  console.log('origSize',_eh.origSize);

  if (_eh.origSize.w <= _eh.wndSize.w && _eh.origSize.h <= _eh.wndSize.h)
  {//меньше чем окно
    _eh.newSize.w = _eh.origSize.w;
    _eh.newSize.h = _eh.origSize.h;
    _eh.newSize.x = Math.floor((_eh.wndSize.w - _eh.origSize.w)/2) + _eh.step;
    _eh.newSize.y = Math.floor((_eh.wndSize.h - _eh.origSize.h)/2) + _eh.step;
  }
  else if (_eh.origSize.w > _eh.wndSize.w && _eh.origSize.h <= _eh.wndSize.h)
  {//ширина больше окна
    _eh.newSize.w = _eh.wndSize.w;
    _eh.newSize.h = Math.floor(_eh.newSize.w*_eh.origSize.h/_eh.origSize.w);
    _eh.newSize.x = _eh.step;
    _eh.newSize.y = Math.floor((_eh.wndSize.h - _eh.newSize.h)/2) + _eh.step;
  }
  else if (_eh.origSize.w <= _eh.wndSize.w && _eh.origSize.h > _eh.wndSize.h)
  {//высота больше окна
    _eh.newSize.h = _eh.wndSize.h;
    _eh.newSize.w = Math.floor(_eh.newSize.h*_eh.origSize.w/_eh.origSize.h);
    _eh.newSize.x = Math.floor((_eh.wndSize.w - _eh.newSize.w)/2) + _eh.step;
    _eh.newSize.y = _eh.step;
  }

  /*switch(origSize.w <= origSize.h ? 'portait' : 'album')
  {
    case 'portait':
      if (origSize.h > wSize.h){
        newSize.h = wSize.h;
        newSize.y = 20;
      }
      else
      {
        newSize.h = origSize.h;
        newSize.y = Math.floor((wSize.h-origSize.h)/2)+step;
      }

      newSize.w = Math.ceil(newSize.h*origSize.w/origSize.h);
      newSize.x = Math.floor( (wSize.w-newSize.w)/2 )+step;
      break;

    case 'album':
      if (origSize.w > wSize.w){
        newSize.w = wSize.w;
        newSize.x = 20;
      }
      else
      {
        newSize.w = origSize.w;
        newSize.x = Math.floor((wSize.w-origSize.w)/2)+step;
      }

      newSize.h = Math.ceil(newSize.w*origSize.h/origSize.w);
      newSize.y = Math.floor( (wSize.h-newSize.h)/2 )+step;
      break;
  }*/

  /*$('body').append('<div id="foo"></div>');
   $('#foo').css({position:'absolute',zIndex:9999,
   width:newSize.w, height:newSize.h,top:newSize.y,left:newSize.x,
   background:'rgba(0,0,0,.9)'});*/
  _eh.bodyWidth=_eh.$body.width();
  _eh.$container.show();
  $('html').addClass('wind_opened');
  _eh.$body.css('margin-right', _eh.$body.width() - _eh.bodyWidth);

  var st=scrollTop();

  console.log('wndSize',_eh.wndSize);
  console.log('newSize',_eh.newSize);
  _eh.$newImg.appendTo(_eh.$container)
    .css({position:'absolute',display:'block', top:_eh.curSize.y, left:_eh.curSize.x, width: _eh.curSize.w, height: _eh.curSize.h})
    .animate({left: _eh.newSize.x, top:_eh.newSize.y, width: _eh.newSize.w, height: _eh.newSize.h}, _eh.speed);

  /*_eh.$newImg.click(function(){
    $(this).animate({top:_eh.curSize.y, left:_eh.curSize.x, width: _eh.curSize.w, height: _eh.curSize.h}, _eh.speed, function(){
      _eh.$container.hide();
      $('html').removeClass('wind_opened');
      _eh.$body.css('margin-right',0);
      $(this).remove();
...
function mxSetup(userId){
  ...
  mixpanel.init("14bbe5ece6a7ad1216c5ea30a5194a28");
  if (window.console){console.log('Mix Panel init');}
  if('undefined'!==typeof userId){
    mixpanel.identify(userId);
    if (window.console){console.log('Mix Panel identify id='+userId);}
  }
}
function mxt(eventName, params){
  /*if('undefined'==typeof window.mixpanel)
  {
    if (window.dbgMode){console.log('Mixpanel track: '+eventName, ', params: ',params)}
    return;
  }

  if('undefined'==typeof params) mixpanel.track(eventName);
  else mixpanel.track(eventName, params);*/

    if('undefined'==typeof window._kmq){console.log('Kissmetrics track: ',eventName, ', params: ',params); return;}
    if('undefined'==typeof params) _kmq.push(['record', eventName]);
    else _kmq.push(['record', eventName, params]);
}
function mxGetSourceLink(){
  var l=window.location.href.replace(window.baseUrl,''), res=null;
  if (''==l || 'sites/list'==l) res='раздел Сайты';
  else if ('sites/create'==l) res='раздел Создание сайта';
  else if ('profile/balance'==l) res='раздел Баланс';
  else if (l.indexOf('pages/edit')>0) res='раздел Редактор сайта';
  else res=l;
  l=null;return res;
}


function showImportantMessages(messages){
  if ($('#important-messages').length==0){
  $('body').append('<div id="important-messages"><table><tr><td>' +
    '<div id="imp-mes-wrap"><div id="imp-mes-list"></div><div class="mt2"><div id="imp-mes-btn" onclick="closeImportantMessages();">OK</div></div></div>' +
    '</td></tr></table></div>');
  } else {
    $('#important-messages .imp-mes').remove();
    $('#important-messages').show();
  }

  var list=$('#imp-mes-list');
  for (var k in messages){
    list.append('<div class="imp-mes" id="mes-'+messages[k]['id_message']+'">' +
      '<div class="date">'+messages[k]['d_date']+'</div>' +
      '<div class="mes">'+messages[k]['mes']+'</div>' +
      '</div>');
  }

  if (!$('html').hasClass('wind_opened')){
...
function closeImportantMessages(){
  $('#important-messages').hide();
  $('html').removeClass('wind_opened');
  $('body').css('margin-right',0);
  var removed=[];
  $('#important-messages .imp-mes').each(function(){
    removed.push(parseInt($(this).attr('id').replace(/^mes-/,'')));
  });
  ajaxLoad({url: 'profile/ajax/frm:remove_messages', data:{ids:removed}});
  removed=null;
}
...
function insertPixel(link) {
    $(document).ready(function(){
        $('body').append($('<img src="'+link.replace('{order_id}', getURLParam('order_id'))+'" width="1" height="1" style="position: absolute;left:-9999px;"/>'));
    });
}
...
/*ROOT/web/js/jquery/jquery.maskedinput.min.js*/
/*
...
/*ROOT/web/js/jquery/jquery.autogrow.js*/
(function ($) {
